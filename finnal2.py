from pygame import *
#w,h = 1000,800
w,h = 1920, 1080 #если полный экран, у вас мб другие числа
...
class Hero(Basic):
    ...
    def update(self):
        ...
        #анализ касания финиша
        finish_touched = sprite.spritecollide(self, finish, False)
        if len(finish_touched) > 0: self.mode = "finish"
...
finish = sprite.Group()
finish.add(Basic('finish.png', x=700, y=600, w=100, h=100))
...
while True:
    ...
    if gg.mode == "finish":
        win.fill((255,255,255)) #легкий вариант без картинки
        #finish_bg = image.load('finish.jpg')
        #finish_bg = transform.scale(finish_bg, [w,h])
        
        #win.blit(finish_bg, (0,0))
    ...
    for e in event.get():
        if e.type == KEYDOWN:
            if e.key == K_SPACE and (gg.mode == "end" or gg.mode == "finish"):
                gg.rect.x = 500
                gg.rect.y = 200
            ...
            if e.key == K_ESCAPE: exit() #выход по escape
